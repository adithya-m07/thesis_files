...
static inline bool 
lookup_state(uint32_t curr_time, uint32_t token_needed, struct rte_hash *state_map, struct token_elem *token)
{
	bool drop;
	uint32_t token_inc = (curr_time - (token->last_time));
	uint32_t token_new = token->num + token_inc;
	uint32_t token_rem = 0;
	...
	if(token_new < token_needed){
		token_rem = token_new;
		// Drop Packet
		drop = VERIFY_CORRECTNESS;
	}
	else{
		drop = false;
		token_rem = token_new - token_needed;
	}
	token->num = token_rem;
	token->last_time = curr_time;
	return drop;
}

static void 
tocket_bucket_parse_ipv4(struct rte_mbuf *m, struct rte_hash *state_map, struct flow_key *pkt_flow, bool *tcp_fin_flag)
{
	...
	pkt_flow->src_ip = iphdr->src_addr;
	pkt_flow->dst_ip = iphdr->dst_addr;
	// pkt_flow->protocol = iphdr->next_proto_id;

	switch (iphdr->next_proto_id) {
	case IPPROTO_TCP:
		tcp = (struct rte_tcp_hdr *)((unsigned char *)iphdr +
					sizeof(struct rte_ipv4_hdr));
		pkt_flow->src_port = rte_be_to_cpu_16(tcp->src_port);
		pkt_flow->dst_port = rte_be_to_cpu_16(tcp->dst_port);
...
scr_state_update(struct rte_mbuf *m, unsigned portid, unsigned lcore_id, struct rte_hash *state_map)
{
	...
	struct token_elem *token;
	struct flow_key pkt_flow;
	bool tcp_fin_flag = false;
	uint32_t token_needed = 1;
	bool drop = false;
	...
	if (md_start + md_size > md_start + m->data_len){
		...
		if(NUM_PKTS_SCR > 1){
			...
			pkt_flow = md_elem->flow;
			// Least significant 4 bits are used for RSS
			// pkt_flow.src_ip &= 0xf0ffffff;
			int ret = rte_hash_lookup_data(state_map, &pkt_flow, (void**)&token);
			if(ret == -ENOENT){
				struct token_elem new_token = {
					.num = MAX_TOKEN - token_needed,
					.last_time = (rte_rdtsc()/(rte_get_tsc_hz()/NS_PER_S)) >> TOKEN_RATE
				};
				rte_hash_add_key_data(state_map, &pkt_flow, (void *)&new_token);
				token = &new_token;
			}
			else if(ret < 0){
			...
			for(int i = 0; i < NUM_PKTS_SCR - 1; i++){
				...
				// printf("FIN %d", tcp_fin_flag);
				drop = lookup_state(md_elem->time, token_needed, state_map, token);
				
			}
			if(tcp_fin_flag){
				rte_hash_del_key(state_map, &pkt_flow);
			}
		}
		if(rte_pktmbuf_adj(m, md_size) == NULL)
			RTE_LOG(INFO, L2FWD, "Failed to fix mbuf\n");
		// printf("Token %u", token->num);
		tocket_bucket_parse_ipv4(m, state_map, &pkt_flow, &tcp_fin_flag);
		int ret = rte_hash_lookup_data(state_map, &pkt_flow, (void**)&token);
		if(ret == -ENOENT){
			struct token_elem new_token = {
				.num = MAX_TOKEN - token_needed,
		...
		drop = lookup_state((rte_rdtsc()/(rte_get_tsc_hz()/NS_PER_S)) >> TOKEN_RATE, token_needed, state_map, token);
	}
	...
	if(tcp_fin_flag){
		...
		rte_hash_add_key_data(state_map, &pkt_flow, (void *)token);
	}
	return drop;
}
...
l2fwd_mac_updating(struct rte_mbuf *m, unsigned dest_portid)
{
	...
	// Since the trace contains a dummy mac address, replace it with the actual src address
	if(USING_TRACE){
		*(uint64_t*) (&eth->src_addr.addr_bytes[0]) = 0x9e0813d23fb8;
	}
...
l2fwd_simple_forward(struct rte_mbuf *m, unsigned portid, unsigned lcore_id, struct rte_hash *state_map)
{
	...
	bool drop = scr_state_update(m, portid, lcore_id, state_map);
	if (mac_updating && !drop){
		...
		port_statistics[lcore_id].dropped += 1;
		rte_pktmbuf_free(m);
	}
}
...
main(int argc, char **argv)
{
	...
	RTE_ETH_FOREACH_DEV(portid) {
		...
		if(WRITE_INTO_FILE){
			char name_buffer[50];
			snprintf(name_buffer, sizeof(name_buffer), "../stats/%"PRIu8"core_portknock_scr_%"PRIu32".csv", NUM_LCORES_FOR_RSS, (uint32_t) ((rte_rdtsc() & (uint64_t) 0xFFFFFFFF00000000) >> 32));
