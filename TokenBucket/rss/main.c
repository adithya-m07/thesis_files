...
static inline bool 
lookup_state(struct flow_key pkt_flow, bool tcp_fin_flag, struct rte_hash *state_map)
{
	...
	if(ret == -ENOENT){
		...
		uint32_t token_inc = ((curr_time - token->last_time)/(rte_get_tsc_hz()/NS_PER_S)) >> TOKEN_RATE;
...
static bool 
tocket_bucket_parse_ipv4(struct rte_mbuf *m, struct rte_hash *state_map)
{
	...
	return lookup_state(pkt_flow, tcp_fin_flag, state_map);
}
...
l2fwd_simple_forward(struct rte_mbuf *m, unsigned portid, unsigned lcore_id, struct rte_hash *state_map)
{
	...
	bool drop = tocket_bucket_parse_ipv4(m, state_map);
