...
/* Simple forward. 8< */
static void
l2fwd_simple_forward(struct rte_mbuf *m, unsigned portid, unsigned lcore_id, struct rte_hash *state_map, unsigned tempj)
{
	...
	while (!force_quit) {
		...
		for (i = 0; i < qconf->n_rx_port; i++) {
			...
			for (j = 0; j < nb_rx; j++) {
				...
				l2fwd_simple_forward(m, portid, lcore_id, state_map, tempj);
...
main(int argc, char **argv)
{
	...
	RTE_ETH_FOREACH_DEV(portid) {
		...
		if(WRITE_INTO_FILE){
			...
			snprintf(name_buffer, sizeof(name_buffer), "../stats/%"PRIu8"core_portknock_rss_%"PRIu32".csv", NUM_LCORES_FOR_RSS, (uint32_t) ((start_tsc_for_file & (uint64_t) 0xFFFFFFFF00000000) >> 32));
	...
	// char name_buffer[100];
	// snprintf(name_buffer, sizeof(name_buffer), "../stats/%"PRIu8"core_portknock_rss_latency_%"PRIu32".csv", NUM_LCORES_FOR_RSS, (uint32_t) ((start_tsc_for_file & (uint64_t) 0xFFFFFFFF00000000) >> 32));
	// log_file = fopen(name_buffer, "w");
	// uint8_t i;
	// for(i = 0; i < NUM_LCORES_FOR_RSS; i++){
